...
for i in lines_english:
    ...
    for j in lines_korean:
        ...
        if line_split_en[0] == line_split_ko[0]:    #enlish과 korean 이름이 같은 것부터 찾기
            for k in lines_math:                    #그 안에서 같은 이름의 math를 찾으면 0(n^3)에서 0(n^2)로 복잡도 감소
                line_split_ma = k.split()
                if line_split_en[0] == line_split_ma[0]:
